...
import shortUrlerAPI from '../API/shortUrlerAPI';
...
const signup = dispatch => async ({ email, password }) => {
    try {
        const response = await shortUrlerAPI.post('/signup', { email, password });
...
const signin = dispatch => async ({ email, password }) => {
    try {
        const response = await shortUrlerAPI.post('/signin', { email, password });
        if (response.status === 200) {
            localStorage.setItem('token', response.data.token);
            localStorage.setItem('email', email);
            dispatch({ type: 'signin', payload: { token: response.data.token, email } });
...
const forgotPassword = dispatch => async ({ email }) => {
    try {
        const response = await shortUrlerAPI.post('/forgotPassword', { email });
        dispatch({ type: 'is_Loading' });
        if (response.status === 200) {
...
const resetPassword = dispatch => async ({ email, password, token }) => {
    try {
        const response = await shortUrlerAPI.post('/resetPassword', { email, password, token });
        dispatch({ type: 'is_Loading' });
        if (response.status === 200) {
