...
const updateDashboard = dispatch => async () => {
    try {
        dispatch({ type: 'is_Loading' });
        const response = await shortUrlerAPI.get('/url/dashboard');
...
const updateUrlsList = dispatch => async () => {
    try {
        dispatch({ type: 'is_Loading' });
        const response = await shortUrlerAPI.get('/urls');
        if (response.status === 200) {
...
const addUrl = dispatch => async ({ url }) => {
    try {
        dispatch({ type: 'is_Loading' });
        const response = await shortUrlerAPI.post('/urls', { url });
        if (response.status === 200) {
            if (response.data) {
...
const deleteUrl = dispatch => async ({ urlId }) => {
    try {
        dispatch({ type: 'is_Loading' });
        const response = await shortUrlerAPI.delete(`/${urlId}`);
        if (response.status === 200) {
            if (response.data) {
